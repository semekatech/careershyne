  ...
  <section class="bg-gradient-to-br from-green-50 via-white to-purple-100 py-20">
    <div class="max-w-7xl mx-auto px-6 lg:px-8">
      <div class="grid md:grid-cols-2 gap-12 items-start">
        <!-- LEFT: Upload CV Form -->
        <div class="bg-white dark:bg-gray-800 rounded-2xl shadow-xl p-10 border border-orange-100 dark:border-gray-700">
          ...
          >
            <svg class="w-14 h-14 text-orange-500 mb-3" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
              <path stroke-linecap="round" stroke-linejoin="round" d="M12 16v-8m-4 4h8M20 12c0 4.418-3.582 8-8 8s-8-3.582-8-8 3.582-8 8-8 8 3.582 8 8z"/>
            </svg>
          ...
          <!-- Attachment Progress -->
          <div v-if="fileName" class="mt-6 bg-gray-50 dark:bg-gray-700/50 rounded-lg p-4">
            <p class="text-sm text-gray-700 dark:text-gray-300">
              📂 Selected File: <strong>{{ fileName }}</strong>
            </p>

            <div class="w-full bg-gray-200 dark:bg-gray-700 rounded-full h-2 mt-3 overflow-hidden">
              <div class="bg-gradient-to-r from-orange-400 to-pink-500 h-2 rounded-full transition-all duration-700 ease-out"
                   :style="{ width: attachmentProgress + '%' }">
              </div>
            </div>
            <p class="text-xs mt-1 text-gray-500 dark:text-gray-400">{{ attachmentProgress }}%</p>
          </div>
          ...
          <div class="mt-8">
            ...
            <div v-if="submitting" class="w-full bg-gray-200 dark:bg-gray-700 rounded-full h-2 mt-3 overflow-hidden">
              <div class="bg-gradient-to-r from-orange-400 to-pink-500 h-2 rounded-full transition-all duration-700 ease-out"
        ...
        <!-- RIGHT: CV Feedback -->
        <div class="bg-white dark:bg-gray-800 rounded-2xl shadow-xl p-10 border border-gray-100 dark:border-gray-700 min-h-[380px] flex flex-col overflow-y-auto">
          <div v-if="review" class="w-full">
            <h4 class="text-lg font-semibold text-orange-600 dark:text-orange-400 mb-4 flex items-center gap-2">
              <span>✨ AI Insights</span>
            </h4>
            <div class="bg-gray-50 dark:bg-gray-700/40 p-4 rounded-lg shadow-inner">
              <pre class="whitespace-pre-wrap text-sm leading-relaxed text-gray-700 dark:text-gray-200">{{ review }}</pre>
            </div>
          </div>

          <div v-else class="w-full">
            <h4 class="font-semibold text-orange-500 mb-4">📊 What you’ll get:</h4>
