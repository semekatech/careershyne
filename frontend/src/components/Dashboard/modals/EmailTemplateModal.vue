<template>
  <div class="fixed inset-0 bg-black bg-opacity-60 flex justify-center items-center z-50 overflow-auto p-4">
    <div class="bg-white w-full md:max-w-4xl h-[80vh] rounded-xl shadow-xl flex flex-col overflow-hidden">
      ...
      <div class="flex justify-between items-center p-4 border-b bg-white sticky top-0 z-10">
        ...
        <div class="flex items-center gap-3">
          <button v-if="editor" @click="downloadWord" class="text-green-600 hover:text-green-800">
            <span class="material-icons">download</span>
          </button>
          <button v-if="editor" @click="copyToClipboard" class="text-blue-600 hover:text-blue-800">
      ...
      <div class="flex-1 p-4 overflow-y-auto bg-gray-50">
        <EditorContent
          v-if="editor"
          :editor="editor"
          class="min-h-[60vh] prose prose-sm max-w-full focus:outline-none"
        />
        <div v-else-if="props.result?.error" class="text-red-600 font-medium bg-red-50 border border-red-200 p-4 rounded-lg">
          {{ props.result.error }}
        </div>
      </div>
    </div>
  ...
  async (newVal) => {
    ...
    if (!editor.value) {
      editor.value = new Editor({
        ...
        content: newVal.template,
      });
    } else {
      editor.value.commands.setContent(newVal.template, false);
    }
  },
  { immediate: true }
);
...
function copyToClipboard() { if (!editor.value) return; navigator.clipboard.writeText(editor.value.getHTML()).then(() => alert("Copied!")); }
