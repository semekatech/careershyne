    ...
    <div class="bg-white w-full md:max-w-4xl h-[80vh] relative mx-2 md:mx-0 rounded-2xl shadow-xl flex flex-col" @click.stop>
      ...
      <div class="flex justify-between items-center p-5 border-b sticky top-0 bg-white z-10 rounded-t-2xl">
        ...
        <div class="flex items-center gap-3">
          <button v-if="editor" @click="downloadWord" class="material-icons text-green-600 hover:text-green-800 cursor-pointer" title="Download as Word">download</button>
          <button v-if="editor" @click="copyToClipboard" class="material-icons text-blue-600 hover:text-blue-800 cursor-pointer" title="Copy to Clipboard">content_copy</button>
      ...
      <div v-if="editor && progress >= 100" class="flex flex-wrap gap-2 p-3 border-b bg-gray-50 items-center">
        ...
        <!-- Headings -->
        <button @click="setHeading(1)" :class="toolbarBtnClass(isHeading1)">H1</button>
        <button @click="setHeading(2)" :class="toolbarBtnClass(isHeading2)">H2</button>
        <button @click="setHeading(3)" :class="toolbarBtnClass(isHeading3)">H3</button>
        ...
        <button @click="toggleBlockquote" :class="toolbarBtnClass(isBlockquote)"><span class="material-icons text-sm">format_quote</span></button>
        <button @click="insertHorizontalRule" class="p-2 rounded-md hover:bg-gray-200" title="Horizontal Line">—</button>
        ...
        <!-- Text align -->
        <button @click="setTextAlign('left')" class="p-2 rounded-md hover:bg-gray-200" title="Align Left"><span class="material-icons text-sm">format_align_left</span></button>
        <button @click="setTextAlign('center')" class="p-2 rounded-md hover:bg-gray-200" title="Align Center"><span class="material-icons text-sm">format_align_center</span></button>
        <button @click="setTextAlign('right')" class="p-2 rounded-md hover:bg-gray-200" title="Align Right"><span class="material-icons text-sm">format_align_right</span></button>
        <button @click="setTextAlign('justify')" class="p-2 rounded-md hover:bg-gray-200" title="Justify"><span class="material-icons text-sm">format_align_justify</span></button>

        <!-- Links -->
        <button @click="setLink" class="p-2 rounded-md hover:bg-gray-200" title="Insert Link"><span class="material-icons text-sm">link</span></button>
      </div>

      <!-- Editor / Loading -->
      <div class="flex-1 p-6 overflow-y-auto bg-gray-50">
        <div v-if="!result || progress < 100" class="flex flex-col items-center justify-center h-full text-center space-y-4">
          <p class="text-gray-600 text-lg font-medium">Generating cover letter<span class="inline-block animate-pulse">...</span></p>
...
watch(() => props.result?.coverLetter, async val => {
  if (!val) return;
  await nextTick();
  if (!editor.value) {
  ...
  } else editor.value.commands.setContent(val, false);
}, { immediate: true });
...
// Loading messages & gradient colors
const messages = [
